...
export async function findAvailableTimeSlots(userId: string, date: string, durationMinutes = 30): Promise<any[]> {
  try {
    ...
    for (const event of dayEvents as CalendarEvent[]) {
      ...
      if (effectiveStart.getTime() - currentTime.getTime() >= durationMinutes * 60 * 1000) {
        availableSlots.push({
          start: currentTime.toISOString(),
          end: effectiveStart.toISOString(),
          duration: Math.floor((effectiveStart.getTime() - currentTime.getTime()) / (1000 * 60)),
        })
      }
    ...
    if (workEnd.getTime() - currentTime.getTime() >= durationMinutes * 60 * 1000) {
      availableSlots.push({
        start: currentTime.toISOString(),
        end: workEnd.toISOString(),
        duration: Math.floor((workEnd.getTime() - currentTime.getTime()) / (1000 * 60)),
      })
    }
...
export async function checkForConflicts(userId: string, start: string, end: string): Promise<boolean> {
  try {
...
export async function analyzeBusyTimes(userId: string, startDate: string, endDate: string): Promise<any> {
  try {

...
async function executeToolCall(userId: string, toolName: string, args: any) {
  ...
  try {
    ...
    try {
      ...
      return {
        error: true,
        message: toolError instanceof Error ? toolError.message : "Unknown error in tool execution",
      }
    }
  } catch (error) {
...
export async function processCalendarQuery(query: string, userId: string, conversationHistory = "") {
  try {
    ...
    try {
      ...
      if (intent.type === "create_event" || intent.type === "update_event" || intent.type === "delete_event") {
      ...
      } else if (intent.type === "query_events" || intent.type === "find_availability") {
        ...
        if (initialResponse.toolCalls && initialResponse.toolCalls.length > 0) {
          ...
          try {
            ...
            toolResults.forEach((result) => {
              const toolName = result.tool.charAt(0).toUpperCase() + result.tool.slice(1).replace(/([A-Z])/g, " $1")
...
export async function streamCalendarQuery(
  ...
  try {
    ...
    const response = await processCalendarQuery(query, userId, conversationHistory)
...
export async function generateEventSuggestion(description: string): Promise<Partial<CalendarEvent>> {
  try {
    ...
    try {
      ...
      try {
        ...
        if (parsedJson.allDay === undefined) {
          ...
          parsedJson.allDay = startDate.getHours() === 0 && startDate.getMinutes() === 0
        }
    ...
    throw new Error(`Error generating event suggestion: ${error instanceof Error ? error.message : String(error)}`)
  }
}
